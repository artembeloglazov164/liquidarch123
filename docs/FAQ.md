...
1. [Actions](https://github.com/320kgpenguin/macos-liquid-arch/actions)
2. Выберите последний успешный workflow run (зеленая галочка ✓)
3. Прокрутите вниз до **Artifacts**
4. Скачайте `320kgpenguin-iso`
5. Распакуйте ZIP архив

**Примечание:** 
- Artifacts хранятся 7 дней
- ISO >2 ГБ, поэтому не может быть в GitHub Releases (лимит 2 ГБ)

**Альтернатива:** Соберите ISO самостоятельно (см. раздел "Сборка ISO")
...
3. Система установит компоненты ДО загрузки KDE:
   - Latte Dock (панель внизу)
   - Calamares (установщик)
   - Темы macOS (WhiteSur, MacSonoma, Albert)
4. После установки KDE Plasma загрузится автоматически
5. Кликните иконку "Install macOS Liquid Arch" на рабочем столе
6. Следуйте инструкциям Calamares

**Требования:** Интернет соединение при первом запуске

**Примечание:** Компоненты устанавливаются из AUR при первом запуске Live ISO (до загрузки KDE), чтобы избежать проблем с местом на диске при сборке в GitHub Actions.
...
Да, через System Settings → Appearance → Global Theme
...
- **Docker LITE**: ~20-30 минут (первая сборка), ~15 минут (с кэшем)
- **Docker FULL**: ~40-60 минут (требует 60 ГБ места, 8+ ГБ RAM)
- **GitHub Actions**: ~40 минут (только LITE режим)
- **Виртуальная машина**: ~30 минут (LITE) или ~50 минут (FULL)
- **Реальный Arch**: ~20 минут (LITE) или ~40 минут (FULL)

**Важно:** FULL режим требует значительно больше ресурсов!
